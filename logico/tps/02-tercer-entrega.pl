...
% masAtareado(Persona).
cantidadEncargos(Persona,CantidadEncargos) :- findall(Trabajos,encargo(_,Persona,Trabajos),ListaTrabajos),length(ListaTrabajos,CantidadEncargos).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Entrega 3
% Parte 1: Escuadron es un conjunto de personajes respetables que sumen entre todos un nivel de respeto mayor a 15.
/* escuadron(ListadoRespetos) :- findall(Persona,(nivelRespeto(Persona,X),X>9),ListadoPersonajes),
	sumadorDeRespetos(ListadoPersonajes,Niveles,ListadoRespetos),
	Niveles > 15.

sumadorDeRespetos([Persona|Personajes],Niveles,[Persona|Restos]) :-
	nivelRespeto(Persona,Nivel),
	SumaNiveles is Niveles + Nivel,
	sumadorDeRespetos(Personajes,SumaNiveles,Restos).

sumadorDeRespetos([],_,[]). */
respetador([Persona|Personajes],[Persona|Demas]) :- respetador(Personajes,Demas).
respetador([_|Personajes],Demas) :- respetador(Personajes,Demas).
respetador([],[]).

sumadorDeRespetos([Persona|Personajes],SumaNivel) :- nivelRespeto(Persona,Nivel), SumaNivel is Nivel + SumaNivel,sumadorDeRespetos(Personajes,SumaNivel).
sumadorDeRespetos([_|Personajes],SumaNivel) :- sumadorDeRespetos(Personajes,SumaNivel).
sumadorDeRespetos([],_).